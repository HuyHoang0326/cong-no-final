    ...
    <div class="main">
        ...
        <div class="stats">
            ...
            <a href="{{ route('debts.overdueDebtList') }}" class="card">
             ...
             <a href="{{ route('debts.isPaidList') }}" class="card" active>
                <div>
                ...
                <tbody>
                    @foreach ($debts as $debt)
                        <tr class="clickable-row" data-href="{{ route('debts.show', $debt->id) }}">
                            <td>{{ $debt->id }}</td>
                            <td>{{ $debt->invoices_id }}</td>
                            <td>{{ $debt->customer?->name ?? $debt->invoice->customer_name }}</td>
                            <td>{{ number_format($debt->debt_amount, 0, ',', '.') }}</td>
                            <td>{{ $debt->debt_date }}</td>
                            <td> <span
                                    class="
                                    {{ $debt->status == 0 ? 'status-pending' : ($debt->status == 2 ? 'status-success' : 'status-danger') }}">
                                    {{ $debt->status == 0 ? 'Chưa Thanh Toán' : ($debt->status == 2 ? 'Đã Thanh Toán' : 'Quá Hạn') }}
                                </span></td>
                        </tr>
                    @endforeach
                </tbody>
